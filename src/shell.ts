...
interface Shell {
    ...
    os(): OS;
    execOpts(): any;
    exec(cmd: string, opts?: any): Promise<Result>;
    execStreaming(cmd: string, callback: ((proc: ChildProcess) => void) | undefined, opts?: any): Promise<Result>;
    execCore(cmd: string, opts: any, callback?: (proc: ChildProcess) => void, stdin?: string): Promise<Result>;
 ...
 */
async function exec(cmd: string, opts?: any): Promise<Result> {
    try {
        if (opts) {
 ...
 * @return {*}  {Promise<ShellResult>} A ShellResult (with code '-1' when an error occurs)
 */
async function execStreaming(cmd: string, callback: (proc: ChildProcess) => void, opts?: any): Promise<Result> {
    console.log(cmd);
    try {
        if (opts) {
 ...
 * @return {Promise<Result>} A ShellResult
 */
function execCore(cmd: string, opts: any, callback?: ((proc: ChildProcess) => void) | null): Promise<Result> {
...
async function which(bin: string): Promise<string | null> {
    ...
    if (res.code === undefined && res.stdout!== undefined) {
        return res.stdout;
    }
